// ─────────────────────────────────────────────────────────────────────────
//  ld_compat.h — 安卓 long double libm 兼容垫片 (通过 CMake -include 全局强制包含)
//
//  背景:
//  安卓 arm64 的 `long double` 是 128 位 binary128。实测发现 bionic 对
//  binary128 的部分 long double 版数学函数存在缺陷 —— 例如 `sinl(-94.9)`
//  会返回 0 (而 `cosl`、double 版 `sin` 均正常)。这会击穿天文级数计算
//  (llrConv → pty_zty2 → 真太阳时), 使八字排盘在勾选"真太阳时"时日/时柱
//  整体错约 8 小时。
//
//  修复:
//  double (64 位) 精度对本库完全足够 (原版 sxwnl 即 JS double 精度; host/iOS
//  的 long double 本就是 64 位且结果全部正确)。故在安卓上把 long double 版
//  libm 统一路由到可靠的 double 版实现, 彻底绕开 bionic 的 binary128 缺陷。
//
