...
void GeoPostion::init()
{
    std::vector<std::string> JWv(std::begin(JWv_array), std::end(JWv_array));
...
void GeoPostion::JWdecode(const std::string &v,double &jin,double &wei)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        //将char类型的字符转换为其对应的整数ASCII值
        decoded_v[i] = static_cast<int>(v[i]);
        if (decoded_v[i] > 96)
        {
            decoded_v[i] -= 97 - 36;
        }
        else if (decoded_v[i] > 64)
        {
            decoded_v[i] -= 65 - 10;
        }
        else
    ...
    jin = decoded_v[2] + decoded_v[3] / 60.0 + 73 ;
...
JINGWEI GeoPostion::getCityGeoPos(const std::string &pName, const std::string &aName) const
{
    auto key = pName + "-" + aName;
    auto posIter = jwMap.find(key);
    if (posIter != jwMap.end())
    {
