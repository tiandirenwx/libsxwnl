...
void LunarYear::init()
{
    ...
    char delimiter = ',';
    gJNB_ = Util::split(s, delimiter);
}

LunarYear::LunarYear(int year)
{
    year_ = year;
    init();
}
...
std::string LunarYear::getNianHao()
{
    // 取年号
    int i, j;
    int y = year_;
    std::string c, s = "";
    std::vector<std::string> ob = gJNB_;
    for (i = 0; i < ob.size(); i += 7)
    {
        j = std::stoi(ob[i].c_str());
        if (y < j || y >= j + std::stoi(ob[i + 1].c_str()))
        {
            continue;
        }
        // 年号及年次
        c = ob[i + 6] + std::to_string(y - j + 1 + std::stoi(ob[i + 2].c_str())) + "";
        // i为年号元年,i+3朝代,i+4朝号,i+5皇帝,i+6年号
        s += s + (s.size() ? ";" : "") + "[" + ob[i + 3] + "]" + ob[i + 4] + " " + ob[i + 5] + " " + c;
    }
    return s;
}

std::string LunarYear::getNianLiStr()
{
    // 字符串年历生成
    int i, j, y = year_;
    std::string s = "", s1, s2;
    ...
    std::vector<int> vecDx = ssq.getDx();
    ...
    for (i = 0; i < 14; i++)
    {
        ...
        v = so_accurate2(vecHS[i]);
        s2 = "(" + JD::JD2str(v + J2000).substr(9, 11) + ")";
        if (int2(v + 0.5) != vecHS[i])
        {
        ...
        for (j = -2; j < 24; j++)
        {
            ...
            if (qi < vecHS[i] || qi >= vecHS[i + 1])
            {
                continue;
            }
            s1 += "  ";
            s1 += Jqmc[(j + 24) % 24] + JD::JD2str(qi + J2000).substr(6, 5);
